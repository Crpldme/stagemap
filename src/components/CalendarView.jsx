...
function Sel({value,onChange,options}) {
  return <select value={value} onChange={e=>onChange(e.target.value)} style={{background:C.tag,border:'1px solid '+C.border,borderRadius:8,padding:'8px 12px',color:C.text,fontFamily:"'Outfit',sans-serif",fontSize:13,outline:'none',width:'100%'}}>{options.map(o=><option key={o.k} value={o.k}>{o.l}</option>)}</select>;
}
...
function Btn({children,onClick,v='primary',sz='md',disabled,full,style:s={}}) {
  ...
  const vars={
    ...
    green:{background:'linear-gradient(135deg,'+C.green+',#28a050)',color:'#fff'},
  }[v];
...
function EventFormModal({date,event,myId,onSave,onDelete,onClose}) {
  ...
  const [form,setForm]=useState({
    ...
    date_start:event?.date_start?event.date_start.split('T')[0]:(date?date.toISOString().split('T')[0]:''),
    date_end:event?.date_end?event.date_end.split('T')[0]:(date?date.toISOString().split('T')[0]:''),
    time_start:event?.time_start||'09:00',
    time_end:event?.time_end||'17:00',
  ...
  useEffect(()=>{
    if(form.event_type==='availability') set('is_availability',true);
    else set('is_availability',false);
  },[form.event_type]);
    ...
    <div style={{position:'fixed',inset:0,background:'#00000090',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.bg2,border:'1px solid '+C.border,borderRadius:16,maxWidth:500,width:'100%',maxHeight:'90vh',overflow:'auto',boxShadow:'0 40px 100px #00000090'}}>
        <div style={{padding:'15px 22px',borderBottom:'1px solid '+C.border,background:C.card,display:'flex',alignItems:'center',gap:10}}>
          <div style={{flex:1,fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:C.cream}}>{isEdit?"Modifier l'événement":'Nouvel événement'}</div>
...
export function CalendarView({myId,profiles=[]}) {
  ...
  const loadEntries=async()=>{
    ...
    try {
      const from=new Date(year,month-1,1).toISOString();
      const to=new Date(year,month+2,0).toISOString();
      const{data,error}=await supabase.from('calendar_entries').select('*').eq('user_id',myId).gte('date_start',from).lte('date_start',to).order('date_start',{ascending:true});
      if(error)throw error;
      setEntries(data||[]);
    } catch(e){toast.error(e.message);}
    setLoading(false);
